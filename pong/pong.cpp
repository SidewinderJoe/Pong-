...
void draw() {
	...
	// draw score
	drawText(width / 2 - 5, height - 10, int2str(score_left) + ":" + int2str(score_right));
...
void vec2_norm(float& x, float &y) {
	// sets vectors length to 1 (which means that x + y == 1)
...
void updateBall(){
	...
	if (ball_pos_x > racket_right_x &&
		...
		// set fly direction depending on where it hit the racket
		// (t is 0.5 if hit at the top, 0 at center, -0.5 at bottom)
